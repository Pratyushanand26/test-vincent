 ...
 */
export async function commitAllowedPolicies(policiesContext, toolName = "tool") {
    const commitResults = {};
    ...
    for (const [policyName, policyContext] of Object.entries(policiesContext.allowedPolicies)) {
        try {
            if (policyContext?.commit && policyContext?.result) {
                ...
                console.log(`[${toolName}] ✅ Policy ${policyName} commit successful:`, commitResult);
            }
            else {
                console.log(`[${toolName}] ⚠️ Policy ${policyName} missing commit function or result`);
            }
        }
        catch (error) {
            console.error(`[${toolName}] ❌ Error committing policy ${policyName}:`, error);
