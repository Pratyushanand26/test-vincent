...
export const vincentTool = createVincentTool({
  packageName: "@agentic-ai/vincent-tool-native-send" as const,
  toolParamsSchema,
  ...
  precheck: async ({ toolParams }, { succeed, fail }) => {
    ...
    if (amountFloat > 1.0) {
      return fail({
        error:
          "[@agentic-ai/vincent-tool-native-send/precheck] Amount too large (maximum 1.0 ETH per transaction)",
      });
    }
  ...
  execute: async (
    ...
    try {
      ...
      console.log(
        "[@agentic-ai/vincent-tool-native-send/execute] Manually calling policy commit function..."
      );
      ...
      try {
        ...
        if (
          ...
          console.log(
            "[@agentic-ai/vincent-tool-native-send/execute] ✅ Found send limit policy context, calling commit..."
          );
          console.log(
            "[@agentic-ai/vincent-tool-native-send/execute] ✅ Policy evaluation result:",
            sendLimitPolicyContext.result
          );
          ...
          console.log(
            "[@agentic-ai/vincent-tool-native-send/execute] ✅ Available in sendLimitPolicyContext:",
            Object.keys(sendLimitPolicyContext)
          );
          console.log(
            "[@agentic-ai/vincent-tool-native-send/execute] ✅ Calling commit with explicit parameters (ignoring TS signature)..."
          );
          ...
          console.log(
            "[@agentic-ai/vincent-tool-native-send/execute] ✅ Policy commit result:",
            commitResult
          );
        } else {
          console.log(
            "[@agentic-ai/vincent-tool-native-send/execute] ❌ Send limit policy context not found in policiesContext.allowedPolicies"
          );
          console.log(
            "[@agentic-ai/vincent-tool-native-send/execute] ❌ Available policies:",
            Object.keys(policiesContext.allowedPolicies || {})
          );
        }
      } catch (commitError) {
        console.error(
          "[@agentic-ai/vincent-tool-native-send/execute] ❌ Error calling policy commit:",
          commitError
        );
