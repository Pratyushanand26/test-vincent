...
export const vincentPolicy = createVincentPolicy({
    packageName: "@agentic-ai/vincent-policy-send-counter-limit",
    toolParamsSchema,
    userParamsSchema,
    commitParamsSchema,
    ...
    precheck: async ({ toolParams, userParams }, { allow, deny, appId, delegation: { delegatorPkpInfo } }) => {
        console.log("[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🔍 POLICY PRECHECK CALLED");
        console.log("[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🔍 Policy precheck params:", {
            toolParams,
            userParams,
        ...
        try {
            ...
            if (!limitCheck.allowed) {
                ...
                console.log("[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🚫 POLICY PRECHECK DENYING REQUEST:");
                console.log("[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🚫 Deny result:", JSON.stringify(denyResult, null, 2));
                console.log("[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🚫 Current count:", limitCheck.currentCount);
                console.log("[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🚫 Max sends:", Number(maxSends));
                console.log("[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🚫 Limit check result:", JSON.stringify(limitCheck, null, 2));
                console.log("[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🚫 About to call deny() function...");
                const denyResponse = deny(denyResult);
                console.log("[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🚫 POLICY PRECHECK DENY RESPONSE:", JSON.stringify(denyResponse, null, 2));
                return denyResponse;
            }
            const allowResult = {
                currentCount: limitCheck.currentCount,
    ...
    evaluate: async ({ toolParams, userParams }, { allow, deny, appId, delegation: { delegatorPkpInfo } }) => {
        console.log("[@agentic-ai/vincent-policy-send-counter-limit/evaluate] Evaluating send limit policy", {
            toolParams,
            userParams,
        });
        // Only use what we actually need - no defaults in policy logic
        ...
        console.log("[@agentic-ai/vincent-policy-send-counter-limit/evaluate] Evaluated send limit policy", {
            currentCount,
            maxSends,
            remainingSends,
        });
    ...
    commit: async ({ currentCount, maxSends, timeWindowSeconds }, { allow, appId, delegation: { delegatorPkpInfo } }) => {
        const { ethAddress } = delegatorPkpInfo;
        console.log("[@agentic-ai/vincent-policy-send-counter-limit/commit] 🚀 IM COMMITING!");
        ...
        if (checkResponse.shouldReset) {
            console.log(`[@agentic-ai/vincent-policy-send-counter-limit/commit] Resetting counter for ${ethAddress} due to time 
      window expiration`);
            try {
                await resetSendCounter(ethAddress, delegatorPkpInfo.publicKey);
                console.log(`[@agentic-ai/vincent-policy-send-counter-limit/commit] Counter reset successful for ${ethAddress}`);
            }
            catch (error) {
        ...
        try {
            ...
            console.log(`[@agentic-ai/vincent-policy-send-counter-limit/commit] Recording send to contract for ${ethAddress} (appId: ${appId})`);
            // Execute the contract call to increment the counter directly
            console.log(`[@agentic-ai/vincent-policy-send-counter-limit/commit] Calling incrementByAddress(${ethAddress}) on contract ${counterSignatures.address}`);
            ...
            console.log("[@agentic-ai/vincent-policy-send-counter-limit/commit] Policy commit successful", {
                ethAddress,
                newCount,
                maxSends,
                remainingSends,
            ...
            console.error("[@agentic-ai/vincent-policy-send-counter-limit/commit] Error in commit phase:", error);
