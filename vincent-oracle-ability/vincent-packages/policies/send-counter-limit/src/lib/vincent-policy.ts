...
export const vincentPolicy = createVincentPolicy({
  packageName: "@agentic-ai/vincent-policy-send-counter-limit" as const,
  ...
  precheck: async (
    ...
    try {
      ...
      if (!limitCheck.allowed) {
        ...
        console.log(
          "[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🚫 POLICY PRECHECK DENYING REQUEST:"
        );
        console.log(
          "[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🚫 Deny result:",
          JSON.stringify(denyResult, null, 2)
        );
        console.log(
          "[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🚫 Current count:",
          limitCheck.currentCount
        );
        console.log(
          "[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🚫 Max sends:",
          Number(maxSends)
        );
        console.log(
          "[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🚫 Limit check result:",
          JSON.stringify(limitCheck, null, 2)
        );
        console.log(
          "[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🚫 About to call deny() function..."
        );

        const denyResponse = deny(denyResult);
        console.log(
          "[@agentic-ai/vincent-policy-send-counter-limit/precheck] 🚫 POLICY PRECHECK DENY RESPONSE:",
  ...
  commit: async (
    ...
    if (checkResponse.shouldReset) {
      console.log(
        `[@agentic-ai/vincent-policy-send-counter-limit/commit] Resetting counter for ${ethAddress} due to time 
      window expiration`
      );
      try {
        await resetSendCounter(ethAddress, delegatorPkpInfo.publicKey);
        console.log(
          `[@agentic-ai/vincent-policy-send-counter-limit/commit] Counter reset successful for ${ethAddress}`
        );
      } catch (error) {
    ...
    try {
      ...
      console.log(
        `[@agentic-ai/vincent-policy-send-counter-limit/commit] Recording send to contract for ${ethAddress} (appId: ${appId})`
      );

      // Execute the contract call to increment the counter directly
      console.log(
        `[@agentic-ai/vincent-policy-send-counter-limit/commit] Calling incrementByAddress(${ethAddress}) on contract ${counterSignatures.address}`
      );
